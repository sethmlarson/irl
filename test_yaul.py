...
def test_invalid_url(url):
    with pytest.raises(yaul.URLError):
        yaul.URL.parse(url)
...
def test_parse_url_normalization(url, expected_normalized_url):
    ...
    actual_normalized_url = yaul.URL.parse(url).unsplit()
...
parse_url_host_map = [
    (
        ...
        yaul.URL(scheme="http", host="google.com", path="/mail"),
    ),
    (
        "http://google.com/mail/",
        yaul.URL(scheme="http", host="google.com", path="/mail/"),
    ...
    ("google.com/mail", yaul.URL(host="google.com", path="/mail")),
    ("http://google.com/", yaul.URL(scheme="http", host="google.com", path="/")),
    ("http://google.com", yaul.URL(scheme="http", host="google.com")),
    (
        "http://google.com?foo",
        yaul.URL(scheme="http", host="google.com", path="", query="foo"),
    ),
    # Path/query/fragment
    ("", yaul.URL()),
    ("/", yaul.URL(path="/")),
    ("#?/!google.com/?foo", yaul.URL(path="", fragment="?/!google.com/?foo")),
    ("/foo", yaul.URL(path="/foo")),
    ("/foo?bar=baz", yaul.URL(path="/foo", query="bar=baz")),
    (
        "/foo?bar=baz#banana?apple/orange",
        yaul.URL(path="/foo", query="bar=baz", fragment="banana?apple/orange"),
    ),
    (
        "/redirect?target=http://localhost:61020/",
        yaul.URL(path="/redirect", query="target=http://localhost:61020/"),
    ),
    # Port
    ("http://google.com/", yaul.URL(scheme="http", host="google.com", path="/")),
    (
        "http://google.com:80/",
        yaul.URL(scheme="http", host="google.com", port=80, path="/"),
    ),
    ("http://google.com:80", yaul.URL(scheme="http", host="google.com", port=80)),
    # Auth
    (
        "http://foo:bar@localhost/",
        yaul.URL(scheme="http", userinfo="foo:bar", host="localhost", path="/"),
    ),
    (
        "http://foo@localhost/",
        yaul.URL(scheme="http", userinfo="foo", host="localhost", path="/"),
    ),
    (
        "http://foo:bar@localhost/",
        yaul.URL(scheme="http", userinfo="foo:bar", host="localhost", path="/"),
    ...
    ("localhost:80/", yaul.URL(host="localhost", port=80, path="/")),
    ("localhost", yaul.URL(host="localhost")),
    # If it really doesn't look like a host though we aren't fooled
    ("localhost://www.google.com", yaul.URL(scheme="localhost", host="www.google.com")),
    (
        "localhost:pass@www.google.com",
        yaul.URL(userinfo="localhost:pass", host="www.google.com"),
    ),
]
...
non_round_tripping_parse_url_host_map = [
    # Path/query/fragment
    ("?", yaul.URL(path="", query="")),
    ("#", yaul.URL(path="", fragment="")),
    # Path normalization
    ("/abc/../def", yaul.URL(path="/def")),
    # Empty Port
    ("http://google.com:", yaul.URL(scheme="http", host="google.com")),
    ("http://google.com:/", yaul.URL(scheme="http", host="google.com", path="/")),
    # Uppercase IRI
    (
        ...
        yaul.URL(scheme="http", host="xn--knigsgchen-b4a3dun.de", path="/stra%C3%9Fe"),
    ),
    ...
    (
        "http://google.com/\uD800",
        yaul.URL(scheme="http", host="google.com", path="/%ED%A0%80"),
    ),
    (
        "http://google.com?q=\uDC00",
        yaul.URL(scheme="http", host="google.com", path="", query="q=%ED%B0%80"),
    ),
    (
        "http://google.com#\uDC00",
        yaul.URL(scheme="http", host="google.com", path="", fragment="%ED%B0%80"),
    ),
...
def test_parse(url, expected_url):
    returned_url = yaul.URL.parse(url)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("/abc/../def", yaul.URL(path="/def")),
        ("/..", yaul.URL(path="/")),
        ("/./abc/./def/", yaul.URL(path="/abc/def/")),
        ("/.", yaul.URL(path="/")),
        ("/./", yaul.URL(path="/")),
        ("/abc/./.././d/././e/.././f/./../../ghi", yaul.URL(path="/ghi")),
    ],
)
def test_parse_and_normalize_url_paths(url, expected_url):
    actual_url = yaul.URL.parse(url)
    assert actual_url == expected_url
    assert actual_url.unsplit() == expected_url.unsplit()


def test_url_str():
    url = yaul.URL(scheme="http", host="google.com")
...
def test_host_header(url, expected_host_header):
    assert yaul.URL.parse(url).host_header() == expected_host_header
...
url_vulnerabilities = [
    ...
    (
        ...
        yaul.URL(scheme="http", host="google.com", path="", fragment="@evil.com/"),
    ),
    # CVE-2016-5699
    (
        "http://127.0.0.1%0d%0aConnection%3a%20keep-alive",
        yaul.URL(scheme="http", host="127.0.0.1%0d%0aconnection%3a%20keep-alive"),
    ),
    # NodeJS unicode -> double dot
    (
        "http://google.com/\uff2e\uff2e/abc",
        yaul.URL(scheme="http", host="google.com", path="/%EF%BC%AE%EF%BC%AE/abc"),
    ),
    # Scheme without ://
    (
        "javascript:a='@google.com:12345/';alert(0)",
        yaul.URL(scheme="javascript", path="a='@google.com:12345/';alert(0)"),
    ),
    ("//google.com/a/b/c", yaul.URL(host="google.com", path="/a/b/c")),
    # International URLs
    (
...
def test_url_vulnerabilities(url, expected_url):
    if expected_url is False:
        ...
        assert yaul.URL.parse(url) == expected_url
